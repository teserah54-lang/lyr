...
| 12 | Audio normalization | **BELUM** (gelombang 3b) | 0 berkas. **Koreksi catatan lama:** Meld TERNYATA punya normalisasi — `MusicService.setupLoudnessEnhancer()` memakai `android.media.audiofx.LoudnessEnhancer` dengan gain `(-loudnessDb * 100)` mB dijepit −1500..+300 mB, sumber `Format.loudnessDb ?: perceptualLoudnessDb` yang mereka simpan di DB per lagu, sakelar `AudioNormalizationKey`. Lyreon belum menyimpan `loudnessDb` dari respons InnerTube → itu pekerjaan 3b | `MusicService.kt` (setupLoudnessEnhancer ~2135-2215), `innertube` `PlayerResponse.Format.loudnessDb` | S–M |
...
| 5 | ~~#10 skip silence~~ **selesai di gelombang 3**; sisa: #12 audio normalization, #15 reorder queue/playlist | semuanya AudioProcessor/Room; satu area kode |
